...
import { listTranscriptionSharedWith, shareTranscriptionWith, unshareTranscriptionWith } from '../../api/transcriptions';
import type { TranscriptionSharedWithItem } from '../../types/transcriptions';
import { openSnackbar } from '../../api/snackbar';
import { usePermission } from '../../hooks/usePermission';
import UserSelect from '../../components/inputs/UserSelect';

type ShareTranscriptionDialogProps = {
  open: boolean;
  onClose: () => void;
...
export default function ShareTranscriptionDialog({
  open,
  onClose,
  transcriptionId,
  transcriptionTitle = ''
}: ShareTranscriptionDialogProps) {
  ...
  const loadSharedWith = useCallback(() => {
    if (!transcriptionId || !canRead) return;
    setLoading(true);
    listTranscriptionSharedWith(transcriptionId)
      .then(setSharedWith)
      .catch((err: unknown) => {
        const msg =
  ...
  }, [open, transcriptionId, canRead, loadSharedWith]);

  const handleAdd = useCallback(() => {
    if (!transcriptionId || !selectedUser || !canCreate) return;
    const userId = selectedUser.id;
    if (sharedWith.some((s) => s.sharedWithUserId === userId)) {
      openSnackbar({
        open: true,
        message: 'Esta transcrição já está compartilhada com este usuário.',
        variant: 'alert',
        alert: { color: 'warning' }
      } as any);
      return;
    ...
    setAdding(true);
    shareTranscriptionWith(transcriptionId, { userId })
      .then(() => {
        openSnackbar({
          open: true,
          message: 'Compartilhamento adicionado.',
          variant: 'alert',
          alert: { color: 'success' }
        } as any);
        setSelectedUser(null);
  ...
  }, [transcriptionId, selectedUser, canCreate, sharedWith, loadSharedWith]);

  const handleRemove = useCallback(
    (sharedWithUserId: string) => {
      if (!transcriptionId || !canDelete) return;
      setRemovingId(sharedWithUserId);
      unshareTranscriptionWith(transcriptionId, sharedWithUserId)
        .then(() => {
          openSnackbar({
            open: true,
    ...
    [transcriptionId, canDelete, loadSharedWith]
  );
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box sx={{ pr: 1 }}>
          <Typography variant="h6">Compartilhar transcrição</Typography>
          {transcriptionTitle && (
      ...
      <DialogContent dividers>
        ...
        {!canRead ? (
          <Typography variant="body2" color="text.secondary">
            Você não tem permissão para ver com quem esta transcrição está compartilhada.
          </Typography>
        ) : loading ? (
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 2 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              Ninguém além de você tem acesso. Adicione usuários acima para compartilhar.
            </Typography>
          </Box>
        ) : (
